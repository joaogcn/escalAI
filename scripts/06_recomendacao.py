...
def run():
    ...
    if confrontos:
        print("  - Confrontos da rodada atual obtidos com sucesso.")
    else:
        print("  - AVISO: Não foi possível obter os confrontos da rodada. A informação de próximo adversário não estará disponível.")
    ...
    if confrontos:
        df_latest['proximo_confronto'] = df_latest['clube_id'].map(confrontos)
    else:
        df_latest['proximo_confronto'] = "Indisponível"
